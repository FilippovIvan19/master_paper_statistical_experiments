...
from utils.constants import DatasetType, DURATION_MAX_GAP_PAIRS, EXP_RESULTS_PATH, \
    POWER_SHEET, ACCUMULATED_SHEET, PEAK_POWER_CONSUMPTION
...
from utils.preprocessing import generate_sync_signals, \
    generate_async_signals_by_point_count, generate_async_signals
from utils.timing import time_measure


def compute_accum_dist_info(ds: DatasetType, full_key: str, time_delta: int,
                            by_resource_delta: bool = False) -> tuple:
    with time_measure(f'compute_accum_dist_info {ds.name}{full_key}'):
        ...
        if by_resource_delta:
            resource_delta = time_delta * PEAK_POWER_CONSUMPTION
            async_signals = generate_async_signals(accumulated, resource_delta)
            point_count = len(async_signals)
        else:
...
def run_experiment_accum(ds: DatasetType, time_delta: int, out_file: str,
                         part: slice = slice(None), processes: int = 1,
                         by_resource_delta: bool = False) -> None:
    with parallelize(processes, []) as parallelizer:
        for (dur, gap) in DURATION_MAX_GAP_PAIRS[part]:
            ...
            for full_key in full_keys:
                parallelizer.apply_async(compute_accum_dist_info, (ds, full_key, time_delta, by_resource_delta))
    ...
    df.to_excel(EXP_RESULTS_PATH + out_file, sheet_name=ACCUMULATED_SHEET)


def compute_power_dist_info(ds: DatasetType, full_key: str, time_delta: int,
                            by_resource_delta: bool = False) -> tuple:
...
def run_experiment_power(ds: DatasetType, time_delta: int, out_file: str,
                         part: slice = slice(None), processes: int = 1,
                         by_resource_delta: bool = False) -> None:
    with parallelize(processes, []) as parallelizer:
        for (dur, gap) in DURATION_MAX_GAP_PAIRS[part]:
            ...
            for full_key in full_keys:
                parallelizer.apply_async(compute_power_dist_info, (ds, full_key, time_delta, by_resource_delta))

    df = pd.DataFrame(data=parallelizer.storage,
                      columns=['ds', 'key', 'duration', 'max_gap', 'index', 'time_delta',
                               'point_count', 'resource_delta', 'sync_dist', 'async_dist'])
    df.to_excel(EXP_RESULTS_PATH + out_file, sheet_name=POWER_SHEET)
